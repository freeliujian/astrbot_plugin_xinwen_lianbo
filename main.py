...
class DailyNews:
    """单日新闻联播数据结构"""
    date: str  # YYYYMMDD
    date_display: str  # YYYY年MM月DD日
    items: List[NewsItem]
    raw_content: str


@register("xinwen_lianbo", "Your Name", "新闻联播查询插件 - 查询并AI总结新闻联播内容", "1.0.0")
class XinwenLianboPlugin(Star):
    ...
    GITHUB_API_URL = "https://api.github.com/repos/DuckBurnIncense/xin-wen-lian-bo/contents/news"
    ...
    async def _summarize_with_ai(self, news: DailyNews, summary_type: str = "brief") -> str:
        """使用AI总结新闻内容"""
        # 准备提示词
...
..."""
        ...
        try:
            llm_response = await self.context.get_using_provider().text_chat(
                prompt=prompt,
                session_id=f"xinwen_summary_{news.date}"
            )
            return llm_response.completion_text
        except Exception as e:
            logger.error(f"AI总结失败: {e}")
            return f"AI总结失败，请稍后重试。错误: {str(e)}"

    @filter.command("xinwen")
    async def query_news(self, event: AstrMessageEvent, date: str = "today"):
        """
        查询新闻联播
        用法: /xinwen [日期] [--summary] [--detail]
        日期格式: today, yesterday, YYYYMMDD, YYYY-MM-DD
        """
        # 解析日期
    ...
    async def summarize_news(self, event: AstrMessageEvent, date: str = "today", style: str = "brief"):
        """
        AI总结新闻联播
        用法: /xinwen-summary [日期] [style]
        style: brief(简要), detailed(详细), category(分类)
        """
        # 解析日期
        parsed_date = self._parse_date(date)
        if not parsed_date:
        ...
        # 获取新闻
        yield event.plain_result(f"正在获取并总结 {parsed_date} 的新闻联播...")
        ...
        # AI总结
        yield event.plain_result("正在进行AI分析，请稍候...")
        summary = await self._summarize_with_ai(news, summary_type=style)
    ...
    async def search_news(self, event: AstrMessageEvent, *, keyword: str):
        """
        搜索新闻联播内容
        用法: /xinwen-search <关键词>
        """
        if not keyword or len(keyword) < 2:
            yield event.plain_result("搜索关键词至少需要2个字符")
            return
    ...
    async def latest_news(self, event: AstrMessageEvent, days: int = 3):
        ...
        for news in results:
            ...
            for idx, item in enumerate(news.items[:5], 1):  # 只显示前5条
...
| `/xinwen-summary [日期] [style]` | AI总结新闻 | `/xinwen-summary`, `/xinwen-summary today detailed` |
